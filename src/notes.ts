...
/**
 * Parse macOS AppleScript date strings like "Friday, 17 April 2026 at 11:34:40 pm"
 * which new Date() can't handle.
 */
function parseAppleScriptDate(s: string): Date | null {
  // "Friday, 17 April 2026 at 11:34:40 pm"
  // Strip the day name and "at", normalize for Date.parse
  const cleaned = s
    .replace(/^[A-Za-z]+,\s*/, "")   // strip "Friday, "
    .replace(/\s+at\s+/, " ")         // "at" → space
    .trim();
  // "17 April 2026 11:34:40 pm"
...
async function getRecentNoteMeta(limit: number = 50): Promise<Array<{ name: string; modifiedAt: Date }>> {
  ...
  for (const line of raw.split("\n")) {
    ...
    const modifiedAt = parseAppleScriptDate(dateStr);
    if (modifiedAt && !isNaN(modifiedAt.getTime())) {
